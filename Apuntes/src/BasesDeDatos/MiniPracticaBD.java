...
public class MiniPracticaBD {
	...
	private static void editarUsuarios() {
		...
		p.add( bBorrar );
		JButton bVolcar = new JButton( "Volcar usuarios" );
		p.add( bVolcar );
		JButton bEdita = new JButton( "Editar contraseñas" );
		p.add( bEdita );
		...
		bVolcar.addActionListener( new ActionListener() { // Acción de añadir usuario
			@Override
			public void actionPerformed(ActionEvent e) {
				try {
					String com=  "";
					com = "SELECT NICK, PASS FROM USUARIO";
					logger.log( Level.INFO, "BD: " + com );
					rs = s.executeQuery( com );
					while(rs.next()) {
						/*logger.log( Level.INFO,*/System.out.println("Usuario: " + rs.getString("Nick") );
					}
				} catch (SQLException e1) {}
			}
		});
		bEdita.addActionListener( new ActionListener() { // Acción de añadir usuario
			@Override
			public void actionPerformed(ActionEvent e) {
				try {
					String com=  "";
					...
					logger.log( Level.INFO, "BD: " + com );
					rs = s.executeQuery( com );
//					tfUsuario.getText() = rs.setString("nick");
				} catch (SQLException e1) {}
			}
		});
